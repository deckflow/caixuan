...
async function repairArgv(
  ...
  const debug = process.env.CAIXUAN_INTERACTIVE_DEBUG === '1';
  if (err.code === 'commander.missingMandatoryOptionValue') {
    ...
    if (debug) {
      ...
      console.error(`[caixuan] repair mandatory option=${m[1]} leaf=${leaf.name()} found=${Boolean(option)}`);
    }
    if (!option) {
      return null;
    }
  ...
  if (err.code === 'commander.optionMissingArgument') {
    ...
    if (debug) {
      // eslint-disable-next-line no-console
      console.error(`[caixuan] repair optionMissingArg option=${m[1]} leaf=${leaf.name()} found=${Boolean(option)}`);
    }
    if (!option) {
      return null;
    }
  ...
  if (err.code === 'commander.missingArgument') {
    ...
    if (debug) {
      // eslint-disable-next-line no-console
      console.error(`[caixuan] repair missingArgument arg=${argName} leaf=${leaf.name()} found=${Boolean(arg)}`);
    }
    if (!arg) {
      return null;
    }
...
export async function parseWithInteractiveRepair(program: Command, argv: string[]): Promise<void> {
  ...
  if (process.env.CAIXUAN_INTERACTIVE_DEBUG === '1') {
    // eslint-disable-next-line no-console
    console.error(
      `[caixuan] interactive=${interactive} stdinTTY=${Boolean(process.stdin.isTTY)} stdoutTTY=${Boolean(process.stdout.isTTY)} hasDevTty=${fs.existsSync(
        DEV_TTY
      )}`
    );
  }
  ...
  try {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (process.env.CAIXUAN_INTERACTIVE_DEBUG === '1') {
          // eslint-disable-next-line no-console
          console.error(`[caixuan] parse attempt=${attempt} caught=${(e as any)?.code ?? 'unknown'}`);
        }
        ...
        if (!next) {
          if (process.env.CAIXUAN_INTERACTIVE_DEBUG === '1') {
            // eslint-disable-next-line no-console
            console.error('[caixuan] repairArgv returned null');
          }
          throw e;
        }
        current = next;
